...
with main_col:
    ...
    st.markdown(
        ...
        f"padding:10px 16px; border-radius:8px; margin-top:10px; "
        f"display:flex; align-items:center; gap:40px; flex-wrap:wrap;'>"
        f"<div style='color:#ff9900; font-weight:700; font-size:1.4rem; "
        f"letter-spacing:0.05em; text-transform:uppercase; white-space:nowrap; line-height:1.3;'>"
        f"TopStep Limits"
        f"<br><span style='color:#444; font-weight:400; font-size:0.62rem; "
        f"letter-spacing:0.05em;'>{int(PRICE_LIMIT_PCT*100)}% CME &minus; "
        f"{int(TOPSTEP_BUFFER*100)}% buffer = &plusmn;{int(effective_pct*100)}% from settlement</span></div>"
        f"<div style='background:#00c85318; border-left:4px solid #00c853; "
        f"padding:6px 12px; border-radius:5px;'>"
        f"<span style='color:#00c853; font-weight:700; font-size:1.4rem;'>Stop Above</span>"
        f"<span style='color:#f0f0f0; float:right; font-size:1.4rem; font-weight:600; "
        f"margin-left:24px;'>{stop_above:.2f}</span>"
        f"<br><span style='color:#aaa; font-size:0.88rem;'>▲ {dist_above:+.0f} pts from price</span>"
        f"</div>"
        f"<div style='background:#d5000018; border-left:4px solid #d50000; "
        f"padding:6px 12px; border-radius:5px;'>"
        f"<span style='color:#d50000; font-weight:700; font-size:1.4rem;'>Stop Below</span>"
        f"<span style='color:#f0f0f0; float:right; font-size:1.4rem; font-weight:600; "
        f"margin-left:24px;'>{stop_below:.2f}</span>"
        f"<br><span style='color:#aaa; font-size:0.88rem;'>▼ {dist_below:+.0f} pts from price</span>"
        f"</div>"
        f"</div>",
        unsafe_allow_html=True,
    )
